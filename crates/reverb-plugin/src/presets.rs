...
//!
//! Reads JSON preset files from the presets/reverb/ directory
...
/// Find the preset directory. Searches for presets/reverb/ relative to
/// the executable or well-known locations.
pub fn find_preset_dir() -> Option<PathBuf> {
    // Try relative to executable
    if let Ok(exe) = std::env::current_exe() {
        for ancestor in exe.ancestors().skip(1) {
            let candidate = ancestor.join("presets/reverb");
            if candidate.is_dir() {
