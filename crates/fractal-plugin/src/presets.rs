...
//!
//! Reads JSON preset files from the presets/fractal/ directory
...
/// Find the preset directory. Searches for presets/fractal/ relative to
/// the executable or well-known locations.
pub fn find_preset_dir() -> Option<PathBuf> {
    // Try relative to executable
    if let Ok(exe) = std::env::current_exe() {
        for ancestor in exe.ancestors().skip(1) {
            let candidate = ancestor.join("presets/fractal");
            if candidate.is_dir() {
