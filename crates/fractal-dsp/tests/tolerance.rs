...
fn golden_stereo_spread() {
    ...
    check(&out_l, &Golden { rms: 2.812955070568323e-1, peak: 5.250000000000000e-1, head: 1.808496525369657e-2, tail: 4.236460498458691e-2 }, TOL, "stereo_spread_L");
    check(&out_r, &Golden { rms: 2.337255591512093e-1, peak: 5.250000000000000e-1, head: 5.319339372861546e0, tail: -5.278850483477455e0 }, TOL, "stereo_spread_R");
}
...
fn golden_stereo_spectral() {
    ...
    p.layer_spread = 0.5;
    let (out_l, out_r) = render_fractal_stereo(&left, &right, &p);
    check(&out_l, &Golden { rms: 2.671122813886648e-1, peak: 5.250000000000000e-1, head: 1.521913212168144e-2, tail: 2.433209429597216e-2 }, TOL, "stereo_spectral_L");
    check(&out_r, &Golden { rms: 2.430904143002330e-1, peak: 5.250000000000000e-1, head: 2.970137519888560e0, tail: -2.930651737476615e0 }, TOL, "stereo_spectral_R");
}
