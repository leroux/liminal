...
//!
//! Reads JSON preset files from the presets/lossy/ directory
...
/// Find the preset directory. Searches for presets/lossy/ relative to
...
pub fn find_preset_dir() -> Option<PathBuf> {
    ...
    if let Ok(exe) = std::env::current_exe() {
        ...
        for ancestor in exe.ancestors().skip(1) {
            let candidate = ancestor.join("presets/lossy");
            if candidate.is_dir() {
