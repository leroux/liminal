...
fn main() {
    ...
    let plugins: Vec<(&str, &[u8; 16], PathBuf, fn(&str) -> Option<BTreeMap<String, Value>>)> = vec![
        ("Reverb", &REVERB_CID, repo_root.join("presets/reverb"), |json| {
            reverb_dsp::FdnParams::from_json(json).ok().map(|p| reverb_params_to_nih(&p))
        }),
        ("Lossy", &LOSSY_CID, repo_root.join("presets/lossy"), |json| {
            lossy_dsp::LossyParams::from_json(json).ok().map(|p| lossy_params_to_nih(&p))
        }),
        ("Fractal", &FRACTAL_CID, repo_root.join("presets/fractal"), |json| {
...
fn find_repo_root() -> Option<PathBuf> {
    ...
    for c in &candidates {
        ...
        if p.join("presets/reverb").is_dir()
            && p.join("presets/lossy").is_dir()
            && p.join("presets/fractal").is_dir()
        {
    ...
    if let Ok(exe) = std::env::current_exe() {
        for ancestor in exe.ancestors().skip(1) {
            if ancestor.join("presets/reverb").is_dir() {
