...
fn golden_modulated() {
    ...
    let out = render_fdn(&input, &p);
    check(&out, &Golden { rms: 3.602250156462878e-1, peak: 6.306414190907959e-1, head: 3.188891636739190e1, tail: -3.806444804411274e1 }, TOL, "modulated");
}
...
fn golden_full_effects() {
    ...
    let out = render_fdn(&input, &p);
    check(&out, &Golden { rms: 3.543098573071106e-1, peak: 5.975797031356403e-1, head: 3.188891636739190e1, tail: -3.606860323942944e1 }, TOL, "full_effects");
}
