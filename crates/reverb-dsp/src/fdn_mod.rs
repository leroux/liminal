...
/// Read from a delay line with Thiran allpass interpolation (fractional delay).
/// Flat magnitude response — no HF loss during modulation, unlike linear interpolation.
#[inline(always)]
fn read_delay_frac(
    buf: &[f64],
    write_idx: usize,
...
pub fn render_fdn_mod(input: &[f64], params: &FdnParams) -> Vec<f64> {
    ...
    // --- DC blocker ---
    let dc_r = 1.0 - 2.0 * std::f64::consts::PI * 5.0 / params.sample_rate;
    ...
    let mut reads = [0.0; N];
    let mut mixed = [0.0; N];
    let mut allpass_state = [0.0_f64; N];
    ...
    for n in 0..n_samples {
        ...
        for i in 0..N {
            ...
            reads[i] = read_delay_frac(&delay_bufs[i], wi, current_delay, delay_buf_len, &mut allpass_state[i]);
