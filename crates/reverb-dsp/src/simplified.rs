...
use crate::params::{FdnParams, SR, N, MATRIX_TYPES, generate_er_delays, generate_er_gains, generate_er_pans};
...
pub struct ReverbParams {
    ...
    // --- Early Reflections ---
    /// Early reflections level (0–1). 0 = off, 1 = full level.
    pub er_level: f64,

    // --- Output EQ ---
    /// High-cut filter frequency on wet signal (20–20000 Hz).
    pub high_cut: f64,
    /// Low-cut filter frequency on wet signal (20–2000 Hz).
    pub low_cut: f64,
}
...
impl ReverbParams {
    ...
    /// Convert to full `FdnParams` for DSP processing at default sample rate.
    pub fn to_fdn_params(&self) -> FdnParams {
        self.to_fdn_params_sr(SR)
    }

    /// Convert to full `FdnParams` for DSP processing at a given sample rate.
    pub fn to_fdn_params_sr(&self, sample_rate: f64) -> FdnParams {
        ...
        for i in 0..N {
            p.delay_times[i] = (base_ms * DELAY_RATIOS[i] / 1000.0 * sample_rate) as i32;
        }
        let diff_base_ms = base_ms * DIFFUSION_SCALE;
        for i in 0..4 {
            p.diffusion_delays[i] = (diff_base_ms * DIFFUSION_RATIOS[i] / 1000.0 * sample_rate).max(1.0) as i32;
        }

        // --- Decay → feedback_gain ---
        p.feedback_gain = self.decay.clamp(0.0, 1.15);

        // --- Brightness → per-node RT60-based damping ---
        // Scale damping per node by (delay_time / mean_delay_time) so all nodes
        // have the same frequency-dependent RT60 regardless of their delay length.
        let base_damping = MAX_DAMPING * (1.0 - self.brightness.clamp(0.0, 1.0));
        let mean_delay: f64 = p.delay_times.iter().map(|&d| d as f64).sum::<f64>() / N as f64;
        if mean_delay > 0.0 {
            for i in 0..N {
                let ratio = p.delay_times[i] as f64 / mean_delay;
                p.damping_coeffs[i] = (base_damping * ratio).clamp(0.0, 0.999);
            }
        } else {
            p.damping_coeffs = vec![base_damping; N];
        }
        ...
        p.pre_delay = (self.pre_delay_ms.clamp(0.0, 250.0) / 1000.0 * sample_rate) as i32;
        ...
        // --- Early Reflections ---
        let er = self.er_level.clamp(0.0, 1.0);
        p.er_level = er;
        if er > 0.0 {
            p.er_delays = generate_er_delays(p.delay_times[0]);
            p.er_gains = generate_er_gains();
            p.er_pans = generate_er_pans();
        }

        // --- Output EQ ---
        p.wet_high_cut_hz = self.high_cut.clamp(200.0, 20000.0);
        p.wet_low_cut_hz = self.low_cut.clamp(20.0, 2000.0);
    ...
    pub fn from_fdn_params(p: &FdnParams) -> Self {
        ...
        ReverbParams {
            ...
            mod_waveform,
            er_level: p.er_level,
            high_cut: p.wet_high_cut_hz,
            low_cut: p.wet_low_cut_hz,
        }
    }
}
...
mod tests {
    ...
    fn test_brightness_mapping() {
        ...
        s.brightness = 1.0;
        let p = s.to_fdn_params();
        for &d in &p.damping_coeffs {
            assert!(d.abs() < 1e-6, "brightness=1 should give zero damping, got {d}");
        }

        // Brightness 0 → max damping (RT60-scaled per node, some clamped at 0.999)
        s.brightness = 0.0;
        let p = s.to_fdn_params();
        // All coefficients should be high (damped)
        for &d in &p.damping_coeffs {
            assert!(d > 0.5, "brightness=0 should give high damping, got {d}");
        }
        // Shortest delay node should have the base damping value
        let min_idx = p.delay_times.iter().enumerate().min_by_key(|(_, d)| *d).unwrap().0;
        let mean_delay: f64 = p.delay_times.iter().map(|&d| d as f64).sum::<f64>() / N as f64;
        let expected_min = MAX_DAMPING * p.delay_times[min_idx] as f64 / mean_delay;
        assert!((p.damping_coeffs[min_idx] - expected_min).abs() < 0.01,
                "shortest delay node damping: {} vs expected {}", p.damping_coeffs[min_idx], expected_min);
    }
