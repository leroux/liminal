...
pub struct LossyProcessor {
    ...
    comb_bufs: [Vec<f64>; 4],
    comb_y1: [f64; 4],
    comb_write_idx: [usize; 4],
    allpass_buf: Vec<f64>,
    allpass_write_idx: usize,
    reverb_wet: Vec<f64>,
...
impl LossyProcessor {
    ...
    pub fn reset(&mut self) {
        ...
        self.comb_y1 = [0.0; 4];
        self.comb_write_idx = [0; 4];
        self.allpass_buf.fill(0.0);
        self.allpass_write_idx = 0;
    ...
    fn lofi_reverb(&mut self, input: &[f64], params: &LossyParams, n: usize) {
        ...
        for c in 0..4 {
            let d = COMB_DELAYS[c];
            let mut idx = self.comb_write_idx[c];
            for i in 0..n {
            ...
            self.comb_write_idx[c] = idx;
        }

        let ap_g = 0.6_f64;
        let mut ap_idx = self.allpass_write_idx;
        for i in 0..n {
            let delayed = self.allpass_buf[ap_idx];
            let inp = self.reverb_wet[i];
            self.reverb_wet[i] = delayed - ap_g * inp;
            self.allpass_buf[ap_idx] = inp + ap_g * self.reverb_wet[i];
            ap_idx += 1;
            if ap_idx >= ALLPASS_DELAY {
                ap_idx = 0;
            }
        }
        self.allpass_write_idx = ap_idx;
