...
# Point to original preset and audio directories
_PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, os.pardir))
PRESET_DIR = os.path.join(_PROJECT_ROOT, "lossy", "gui", "presets")
DEFAULT_SOURCE = os.path.join(_PROJECT_ROOT, "audio", "test_signals", "dry_noise_burst.wav")
...
class LossyGUI:
    ...
    def _build_log_page(self):
        ...
        ttk.Button(toolbar, text="Copy All", command=self._copy_log).pack(side="right", padx=4)

        self._log_text = tk.Text(f, wrap="word", bg="#1a1a1a", fg="#cccccc",
                                 font=("Menlo", 11), padx=8, pady=8, relief="flat",
                                 selectbackground="#334466", insertbackground="#cccccc")
        scroll = ttk.Scrollbar(f, orient="vertical", command=self._log_text.yview)
        self._log_text.configure(yscrollcommand=scroll.set)
        scroll.pack(side="right", fill="y")
        self._log_text.pack(fill="both", expand=True, padx=4, pady=4)

        self._log_text.tag_configure("error", foreground="#ff6666")
        self._log_text.tag_configure("warning", foreground="#ffcc66")
        self._log_text.tag_configure("info", foreground="#88bb88")
        self._log_text.tag_configure("time", foreground="#666666")

    def _log(self, message, level="error"):
        """Append a message to the log tab. Thread-safe."""
        ts = time.strftime("%H:%M:%S")
        tag = level  # "error", "warning", or "info"
        def append():
    ...
    def _render_and_play(self, params, on_complete=None, status_prefix=None):
        ...
        def do_render():
            try:
                ...
                self._log(tb)
                msg = f"ERROR: {exc}"
                self.root.after(0, lambda msg=msg: (
