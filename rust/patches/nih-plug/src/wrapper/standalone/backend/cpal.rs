...
impl CpalMidir {
        ...
        T: Sample + FromSample<f32>,
    {
        ...
        move |data, _info| {
            ...
            {
                // Even though we told CPAL that we wanted `buffer_size` samples, it may still
                // give us fewer or more (macOS CoreAudio is known to deliver more). Clamp to
                // buffer_size to avoid panicking.
                let actual_sample_count =
                    (data.len() / num_output_channels).min(buffer_size);
                let buffers = unsafe {
            ...
            // If CPAL delivered more samples than buffer_size (macOS CoreAudio), copy what
            // we have and zero-fill the rest.
            ...
            for (i, output_sample) in data.iter_mut().enumerate() {
                ...
                *output_sample = if n < buffer_size {
                    T::from_sample(main_io_storage[ch][n])
                } else {
                    T::EQUILIBRIUM
                };
            }
